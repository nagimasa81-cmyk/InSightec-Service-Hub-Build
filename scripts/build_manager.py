...
HUB_MANIFEST_DEFAULTS = {
 "Complaint_service_hub": {
  "id": "complaint_service_hub",
  "name": {"en":"Complaint Service Hub","ja":"Complaint Service Hub","es":"Complaint Service Hub","zh_TW":"Complaint Service Hub","ko":"Complaint Service Hub"},
  "description": {"en":"Complaint creation and service workflow tool.","ja":"Complaint作成およびサービスワークフローツール","es":"Herramienta de flujo de trabajo de quejas y servicio.","zh_TW":"Complaint 建立與服務工作流程工具","ko":"Complaint 작성 및 서비스 워크플로 도구"},
  "category": "Service Workflow",
 },
 "DO_Analysis": {"id":"do_analysis","name":{"en":"DO Analysis","ja":"DO解析","es":"Análisis DO","zh_TW":"DO 分析","ko":"DO 분석"},"category":"Analysis"},
 "Log_explorer": {"id":"log_explorer","name":{"en":"Log Explorer","ja":"ログエクスプローラー","es":"Explorador de registros","zh_TW":"日誌瀏覽器","ko":"로그 탐색기"},"category":"Analysis"},
 "FFT": {"id":"fus_image_explore","name":{"en":"FUS Image Explore","ja":"FUS画像エクスプローラー","es":"Explorador de imágenes FUS","zh_TW":"FUS 影像探索工具","ko":"FUS 이미지 탐색기"},"category":"FUS Image Analysis"},
 "trackerSNR": {"id":"tracker_snr","name":{"en":"Tracker SNR","ja":"Tracker SNR","es":"Tracker SNR","zh_TW":"Tracker SNR","ko":"Tracker SNR"},"category":"Analysis"},
 "VIMeasure": {"id":"vimeasure","name":{"en":"VIMeasure","ja":"V/I測定","es":"Medición V/I","zh_TW":"V/I 量測","ko":"V/I 측정"},"category":"Analysis"},
 "Soni": {"id":"sonication_analysis","name":{"en":"Sonication Analysis","ja":"Sonication解析","es":"Análisis de sonicación","zh_TW":"Sonication 分析","ko":"Sonication 분석"},"category":"Sonication Analysis"},
}

def _manifest_version(ctx:Context)->str:
 return version_value(ctx.version, ctx.source_zip.name)

def normalize_hub_tool_manifest(module_name:str, ctx:Context, target:Path, exe:Path)->dict:
 """Write the canonical Hub-facing manifest after payload copy/reuse.

 Payload manifests describe the standalone package and are not trusted as Hub
 manifests.  Existing rich Hub metadata is retained where possible, while the
 identity, executable and integration fields are always generated by Framework.
 """
 manifest_path=target/"manifest.json"
 try:
  data=json.loads(manifest_path.read_text(encoding="utf-8-sig")) if manifest_path.is_file() else {}
 except Exception:
  data={}
 if not isinstance(data,dict): data={}
 defaults=HUB_MANIFEST_DEFAULTS[module_name]
 data["id"]=defaults["id"]
 data["name"]=data.get("name") if isinstance(data.get("name"),dict) else defaults["name"]
 # Ensure English name always exists even when a standalone manifest uses a string.
 data["name"].setdefault("en", str(ctx.module_config.get("display_name") or module_name))
 if not isinstance(data.get("description"),dict):
  data["description"]=defaults.get("description", {k:data["name"].get(k,data["name"]["en"]) for k in ("en","ja","es","zh_TW","ko")})
 data["version"]=_manifest_version(ctx)
 data["build"]=time.strftime("%Y-%m-%d",time.gmtime())
 data["exe"]=exe.name
 data["enabled"]=True
 data["status"]="active"
 data["developer"]="InSightec"
 data["category"]=data.get("category") or defaults.get("category","Analysis")
 data["packaging"]="built_from_repository_module"
 data["source_module"]=module_name
 data["source_sha256"]=sha256(ctx.source_zip)
 candidates=[]
 for value in [exe.name, ctx.module_config.get("main_executable"), ctx.module_config.get("smoke_executable"), *(ctx.module_config.get("required_executables",[]) or []), *(data.get("executable_candidates",[]) or []), *(data.get("acceptable_executables",[]) or [])]:
  value=str(value or "").strip()
  if value and value not in candidates: candidates.append(value)
 data["executable_candidates"]=candidates
 data["acceptable_executables"]=list(candidates)
 data.setdefault("executable_patterns",[])
 data.setdefault("executable_exclude",["debug","console","uninstall","updater"])
 data.setdefault("handoff_schema","insightec.auto-analysis.handoff.v1")
 data.setdefault("handoff",{"schema":"insightec.auto-analysis.handoff.v1","argv":"--handoff <json>","environment":["INSIGHTEC_HANDOFF"],"receiver":"native module receiver"})
...
def _module_signature(ctx:Context)->str:
 ...
 raw="|".join([
  ...
  str(ctx.module_config.get("smoke_executable") or ""),"Nuitka==4.1.3","module-cache-v4-manifest-sync"
 ])
 import hashlib
...
def assemble_hub_tools(hub_root:Path,args,reg,hub_ctx:Context)->tuple[list[dict],SonicationPipeline|None]:
 ...
 for name in selected_modules:
  ...
  manifest=normalize_hub_tool_manifest(name,ctx,target,exe)
  results.append({"module":name,"exe":exe.name,"target":str(target.relative_to(hub_root)),"manifest_id":manifest["id"],"qt_deploy":qt_deploy,"reused":reused,"module_signature":module_signature,"seconds":round(time.time()-started,2)})
 ...
 if args.include_sonication:
  ...
  try:
   ...
   manifest=normalize_hub_tool_manifest("Soni",soni_ctx,target,pipeline.exe)
   results.append({"module":"Soni","exe":pipeline.exe.name,"target":str(target.relative_to(hub_root)),"manifest_id":manifest["id"],"pipeline":"dedicated_v1","reused":bool(cached),"module_signature":module_signature})
