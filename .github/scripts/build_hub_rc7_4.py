...
def verify_windows_batch_invocation(temp_root: Path) -> None:
    ...
    if os.name != "nt":
        return
    probe_dir = temp_root / "RC7.4 command probe with spaces"
    shutil.rmtree(probe_dir, ignore_errors=True)
    ...
    bat.write_text(
        "@echo off\r\n"
        "setlocal\r\n"
        f'>"{marker}" echo RC7_4_CMD_OK\r\n'
        "exit /b 0\r\n",
        encoding="ascii",
    )
    code = run_cmd_batch(bat, cwd=probe_dir, env=os.environ.copy(), label="CMD-PROBE")
    if code != 0 or not marker.is_file() or marker.read_text(encoding="ascii").strip() != "RC7_4_CMD_OK":
...
def main() -> int:
    ...
    log("RC7.4 DETERMINISTIC GLOBAL PREFLIGHT — resolve every Module before compiling anything")
    selected: dict[str, Candidate] = {}
    app_roots: dict[str, Path] = {}
    all_errors: list[str] = []
    report: dict[str, Any] = {"schema": "rc7.4-preflight-report/v1", "modules": {}, "errors": []}
    ...
    report_path = workspace / "RC7_4_PREFLIGHT_REPORT.json"
    ...
    if all_errors:
        ...
        for i, error in enumerate(all_errors, 1):
            print(f"  {i:02d}. {error}")
        raise BuildFailure(f"RC7.4 deterministic build preflight failed with {len(all_errors)} issue(s). No compilation was started.")
    print("\n[PASS] All seven Module packages passed the RC7.4 deterministic build global preflight.")
