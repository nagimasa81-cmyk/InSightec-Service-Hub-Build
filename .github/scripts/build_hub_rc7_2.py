...
def run_build(app_root: Path, contract: dict[str, Any], module_id: str) -> None:
    build_file = app_root / contract["build"]["file"]
    for d in contract["build"].get("output_directories", ["dist", "output", "release"]):
        path = app_root / d
        # Preserve a build directory if the BAT itself is inside it.
        try:
    ...
    result = subprocess.run(["cmd.exe", "/d", "/s", "/c", f'call "{build_file}"'], cwd=app_root, env=env)
    if result.returncode:
        raise BuildFailure(f"{module_id}: build BAT failed with exit code {result.returncode}")
...
def main() -> int:
    ...
    log("RC7.2 STRONG GLOBAL PREFLIGHT — resolve every Module before compiling anything")
    selected: dict[str, Candidate] = {}
    app_roots: dict[str, Path] = {}
    all_errors: list[str] = []
    report: dict[str, Any] = {"schema": "rc7.1-preflight-report/v1", "modules": {}, "errors": []}
    ...
    for module_id, folder in MODULES.items():
        ...
        try:
            ...
            validation = validate_contract(chosen, root) + compile_python(root, module_id) + strong_module_checks(chosen, root)
    ...
    report_path = workspace / "RC7_2_PREFLIGHT_REPORT.json"
    ...
    if all_errors:
        ...
        for i, error in enumerate(all_errors, 1):
            print(f"  {i:02d}. {error}")
        raise BuildFailure(f"RC7.2 native-handoff strong preflight failed with {len(all_errors)} issue(s). No compilation was started.")
    print("\n[PASS] All seven Module packages passed the RC7.2 native-handoff strong global preflight.")
    ...
    status_file.write_text(json.dumps({
        ...
        "source": "repository_modules_rc7_2",
