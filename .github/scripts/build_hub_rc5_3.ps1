...
'@
  ...
  # AST-based runtime verification for executable Python only.
  # JSON, translations, version metadata, comments and documentation are intentionally ignored.
  # The build fails only for guide/tour imports or actual executable calls/references.
  $scannerPath = Join-Path $env:RUNNER_TEMP 'rc5_3_guide_runtime_scanner.py'
  $scanner = @'
import ast
import json
import pathlib
...
GUIDE_MODULE = re.compile(
    r"(?:^|\.)(?:common_guide|guide_manager|guided_tour|tour_manager|startup_guide|guide_viewer)(?:\.|$)",
    re.I,
)
GUIDE_CALL = re.compile(
    r"^(?:GuideManager|TourManager|GuidedTour|GuideViewer|StartupGuide|"
    r"show_guide(?:_and_tour)?|start_tour|open_guide|launch_guide|"
    r"handle_startup_guide(?:_flow)?|setup_guide(?:_system)?|create_guide|run_tour)$",
    re.I,
)
...
class RuntimeVisitor(ast.NodeVisitor):
    ...
    def visit_Import(self, node):
        for alias in node.names:
            if GUIDE_MODULE.search(alias.name):
                self.add(node, "guide-import", alias.name)
        self.generic_visit(node)

    def visit_ImportFrom(self, node):
        module = node.module or ""
        imported = [alias.name for alias in node.names]
        if GUIDE_MODULE.search(module):
            self.add(node, "guide-import-from", f"{module}: {', '.join(imported)}")
        else:
            for name in imported:
                if GUIDE_CALL.fullmatch(name):
                    self.add(node, "guide-symbol-import", f"{module}: {name}")
        self.generic_visit(node)

    def visit_Call(self, node):
        name = dotted(node.func)
        if GUIDE_MODULE.search(name) or GUIDE_CALL.fullmatch(leaf_name(name)):
            self.add(node, "guide-runtime-call", name)
        self.generic_visit(node)


skip_parts = {
    "tests", "test", "docs", "doc", "examples", "example", "help",
...
'@
  ...
  if ($scanExit -ne 0) {
    throw "Executable Python guide/tour references remain after release cleanup for $ToolId"
  }

  Write-Host ("[PASS] Python AST runtime scan: no executable guide/tour references: " + $ToolId)
