...
def validate_contract(candidate: Candidate, app_root: Path) -> list[str]:
    ...
    # RC7.5: statically validate Nuitka include-package names before dependencies/build.
    if build_file.is_file():
        ...
        if probes:
            probe_line = "import " + ", ".join(probes)
            need(probe_line.lower() in bat_text.lower(),
                 f"selected BAT lacks post-install decoder import probe: {probe_line}")
...
def verify_windows_batch_invocation(temp_root: Path) -> None:
    ...
    if os.name != "nt":
        return
    probe_dir = temp_root / "RC7.5 command probe with spaces"
    shutil.rmtree(probe_dir, ignore_errors=True)
    ...
    bat.write_text(
        "@echo off\r\n"
        "setlocal\r\n"
        f'>"{marker}" echo RC7_5_CMD_OK\r\n'
        "exit /b 0\r\n",
        encoding="ascii",
    )
    code = run_cmd_batch(bat, cwd=probe_dir, env=os.environ.copy(), label="CMD-PROBE")
    if code != 0 or not marker.is_file() or marker.read_text(encoding="ascii").strip() != "RC7_5_CMD_OK":
...
def main() -> int:
    ...
    log("RC7.5 DETERMINISTIC GLOBAL PREFLIGHT — resolve every Module before compiling anything")
    selected: dict[str, Candidate] = {}
    app_roots: dict[str, Path] = {}
    all_errors: list[str] = []
    report: dict[str, Any] = {"schema": "rc7.5-preflight-report/v1", "modules": {}, "errors": []}
    ...
    report_path = workspace / "RC7_5_PREFLIGHT_REPORT.json"
    ...
    if all_errors:
        ...
        for i, error in enumerate(all_errors, 1):
            print(f"  {i:02d}. {error}")
        raise BuildFailure(f"RC7.5 deterministic build preflight failed with {len(all_errors)} issue(s). No compilation was started.")
    print("\n[PASS] All seven Module packages passed the RC7.5 deterministic build global preflight.")
    ...
    status_file.write_text(json.dumps({
        ...
        "source": "repository_modules_rc7_4",
