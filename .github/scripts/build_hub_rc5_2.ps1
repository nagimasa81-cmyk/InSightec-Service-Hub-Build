...
'@
  ...
  # AST-based runtime verification. Comments, README, CHANGELOG and plain documentation
  # are intentionally ignored. Only executable Python references and active JSON flags fail.
  $scannerPath = Join-Path $env:RUNNER_TEMP 'rc5_2_guide_runtime_scanner.py'
  $scanner = @'
import ast
import json
import pathlib
...
GUIDE_TOKEN = re.compile(r"(?:^|_)(?:common_guide|guide_manager|guided_tour|tour_manager|startup_guide|guide_viewer)(?:$|_)", re.I)
RUNTIME_NAME = re.compile(r"(?:show|start|open|launch|handle|setup|create|build|enable|display|run).*?(?:guide|tour)|(?:guide|tour).*?(?:show|start|open|launch|handle|setup|create|build|enable|display|run)", re.I)
UI_TEXT = re.compile(r"(?:show\s+guide|guide\s*/\s*tour|guided\s+tour|do\s+not\s+ask\s+again|next\s+startup.*guide|quick\s+guide)", re.I)
...
class RuntimeVisitor(ast.NodeVisitor):
    ...
    def visit_Import(self, node):
        for alias in node.names:
            if GUIDE_TOKEN.search(alias.name.replace('.', '_')):
                self.add(node, "import", alias.name)
        self.generic_visit(node)

    def visit_ImportFrom(self, node):
        module = node.module or ""
        names = [a.name for a in node.names]
        combined = "_".join([module, *names]).replace('.', '_')
        if GUIDE_TOKEN.search(combined):
            self.add(node, "import-from", f"{module}: {', '.join(names)}")
        self.generic_visit(node)

    def visit_Call(self, node):
        name = dotted(node.func)
        compact = name.replace('.', '_')
        if GUIDE_TOKEN.search(compact) or RUNTIME_NAME.search(compact):
            self.add(node, "runtime-call", name)
        for arg in [*node.args, *(kw.value for kw in node.keywords)]:
            if isinstance(arg, ast.Constant) and isinstance(arg.value, str) and UI_TEXT.search(arg.value):
                self.add(arg, "runtime-ui-text", arg.value[:160])
        self.generic_visit(node)

    def visit_Name(self, node):
        # Detect executable references, but do not fail solely on definitions.
        if isinstance(node.ctx, ast.Load) and (GUIDE_TOKEN.search(node.id) or RUNTIME_NAME.search(node.id)):
            self.add(node, "runtime-name", node.id)
        self.generic_visit(node)
    ...
    def visit_Attribute(self, node):
        if isinstance(node.ctx, ast.Load):
            ...
            compact = name.replace('.', '_')
            if GUIDE_TOKEN.search(compact) or RUNTIME_NAME.search(compact):
                self.add(node, "runtime-attribute", name)
        self.generic_visit(node)


skip_parts = {"tests", "test", "docs", "examples", "example", "__pycache__", ".pytest_cache", ".mypy_cache"}
for path in root.rglob("*.py"):
    ...
    try:
        ...
        findings.append({"file": str(path.relative_to(root)), "line": getattr(exc, "lineno", 0) or 0, "kind": "parse-error", "value": str(exc)})
        continue
    RuntimeVisitor(path).visit(tree)

# Structured JSON check: only active guide/tour configuration fails.
def walk_json(value, path, trail=""):
    if isinstance(value, dict):
        for key, item in value.items():
            current = f"{trail}.{key}" if trail else str(key)
            key_norm = re.sub(r"[^a-z0-9]", "_", str(key).lower())
            if ("guide" in key_norm or "tour" in key_norm):
                active = item is True or (isinstance(item, (int, float)) and item != 0) or (isinstance(item, str) and item.strip().lower() not in {"", "false", "off", "none", "disabled", "0"})
                if active:
                    findings.append({"file": str(path.relative_to(root)), "line": 0, "kind": "active-json-setting", "value": f"{current}={item!r}"})
            walk_json(item, path, current)
    elif isinstance(value, list):
        for index, item in enumerate(value):
            walk_json(item, path, f"{trail}[{index}]")
...
# Deduplicate because Attribute and Name visitors can overlap.
unique = []
seen = set()
for item in findings:
    key = (item["file"], item["line"], item["kind"], item["value"])
    if key not in seen:
        seen.add(key)
        unique.append(item)

print(json.dumps(unique, ensure_ascii=False, indent=2))
sys.exit(2 if unique else 0)
'@
  Set-Content -LiteralPath $scannerPath -Value $scanner -Encoding UTF8

  $scanOutput = & python $scannerPath $Root 2>&1
  $scanExit = $LASTEXITCODE
  if ($scanOutput) { $scanOutput | ForEach-Object { Write-Host $_ } }
  if ($scanExit -ne 0) {
    throw "Executable guide/tour references remain after release cleanup for $ToolId"
  }

  Write-Host ("[PASS] AST runtime scan: no executable guide/tour references: " + $ToolId)
